...
function OosPerformanceChartInner({ model, equity, totalPeriods, currentPeriod, bestTrial, periods }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] min-w-[60px] text-right" style={{ fontFamily: "var(--font-mono)", color: "var(--color-text-secondary)" }}>
        ...
        {lastModel !== null && (
          <div className="flex flex-col">
            ...
            <span className="text-sm font-semibold" style={{ fontFamily: "var(--font-mono)", color: "var(--color-text-primary)" }}>
              {yMode === "pct" ? `${((lastModel! - 1) * 100).toFixed(2)}%` : `$${(lastModel! * 10000).toFixed(0)}`}
            </span>
          </div>
        )}
        {lastBh !== null && (
          <div className="flex flex-col">
            ...
            <span className="text-sm font-semibold" style={{ fontFamily: "var(--font-mono)", color: "var(--color-text-secondary)" }}>
              {yMode === "pct" ? `${((lastBh! - 1) * 100).toFixed(2)}%` : `$${(lastBh! * 10000).toFixed(0)}`}
            </span>
          </div>
        )}
      </div>
        ...
        <div className="flex gap-3 flex-wrap">
          {(() => {
            ...
            const cards = [
              ...
              { label: "Return", value: lp.return_pct != null ? `${lp.return_pct.toFixed(2)}%` : "-" },
              { label: "Trades", value: lp.trades?.toString() ?? "-" },
              { label: "Drawdown", value: lp.drawdown?.toFixed(2) ?? "-" },
              { label: "Win Rate", value: lp.win_rate != null ? `${(lp.win_rate * 100).toFixed(1)}%` : "-" },
            ];
            return cards.map((c) => (
              <div
                key={c.label}
                className="flex flex-col rounded-md border px-2.5 py-1.5"
                style={{ borderColor: "var(--color-border)", backgroundColor: "var(--color-elevated)", minWidth: 72 }}
              >
                <span className="text-[9px] uppercase tracking-[0.08em]" style={{ color: "var(--color-text-muted)" }}>
                  {c.label}
                </span>
                <span className="text-xs font-semibold" style={{ fontFamily: "var(--font-mono)", color: "var(--color-text-primary)" }}>
                  {c.value}
                </span>
              </div>
            ));
