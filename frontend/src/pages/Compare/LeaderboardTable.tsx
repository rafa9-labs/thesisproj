...
function ReturnRenderer({ value }: { value: number | null }) {
  if (value == null) return <span className="text-(--color-text-muted)">—</span>;
  ...
  const severity =
    Math.abs(value) < 0.1
      ...
      : Math.abs(value) < 0.2
        ...
        : "var(--color-accent-danger)";
  return (
    <span className="font-mono" style={{ color: severity, fontSize: 12 }}>
      {formatPercent(value)}
    </span>
  );
}
...
export function LeaderboardTable({ metrics = [], sortMetric = "sharpe" }: LeaderboardTableProps) {
  ...
  if (metrics.length === 0) {
    return (
      <div className="flex items-center justify-center rounded-sm border border-(--color-glass-border) bg-(--color-glass) p-8 text-(--color-text-muted) backdrop-blur-[12px]">
        <span className="font-mono text-sm font-light">No metrics data available</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div
        className="ag-theme-alpine-dark overflow-hidden rounded-sm border backdrop-blur-[12px]"
        style={
          {
            width: "100%",
