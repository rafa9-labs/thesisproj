    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap items-start gap-4">
        <div className="flex flex-col gap-1">
          <label className="text-[10px] font-semibold tracking-wide text-(--color-text-muted) uppercase">
            Model
          </label>
          <select
            value={effectiveModel}
            onChange={(e) => setSelectedModel(e.target.value)}
            className="rounded-md border border-(--color-border) bg-(--color-surface) px-2 py-1.5 font-mono text-xs text-(--color-text-primary)"
            style={{ minWidth: 160 }}
          >
            {models.map((m) => (
          ...
          <div className="flex flex-wrap gap-1">
            {pairs.map((p) => (
              <button
                ...
                style={{
                  borderColor: selectedPairs.includes(p)
                    ? "var(--color-accent)"
                    : "var(--color-border)",
                  backgroundColor: selectedPairs.includes(p)
                    ? "rgba(41,98,255,0.1)"
                    : "var(--color-surface)",
                  color: selectedPairs.includes(p)
                    ? "var(--color-accent)"
        ...
        <div className="flex items-end">
          <button
            ...
            className="rounded-md border border-(--color-accent) px-3 py-1.5 font-mono text-xs font-semibold transition-colors"
            style={{
              backgroundColor:
                selectedPairs.length < 2 || loading
                  ? "var(--color-surface)"
                  : "var(--color-accent)",
